import os
import sys
from playwright.sync_api import sync_playwright

url = "https://www.linovelib.com/novel/8/1843.html"

with sync_playwright() as p:
    browser = p.chromium.launch(
        headless=False,
        args=[
            "--disable-blink-features=AutomationControlled",
            "--window-position=-32000,-32000",
            "--window-size=10,10"
        ]
    )
    context = browser.new_context(
        user_agent="Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/125.0.0.0 Safari/537.36",
        viewport={"width": 1024, "height": 768}
    )
    page = context.new_page()
    page.goto(url, timeout=20000, wait_until="domcontentloaded")
    
    # Wait for decryption
    page.wait_for_function(
        "() => { "
        "  const el = document.querySelector('div#mlfy_main_text'); "
        "  if (!el || !el.innerText) return false; "
        "  const text = el.innerText; "
        "  return !text.includes('內容加載失敗') && "
        "         !text.includes('内容加载失败') && "
        "         !text.includes('数据缺失') && "
        "         !text.includes('正在加载'); "
        "}",
        timeout=15000
    )
    
    # Inspect paragraphs inside #TextContent and print their classes
    html_content = page.evaluate("() => document.getElementById('TextContent').innerHTML")
    with open("scratch/live_TextContent.html", "w", encoding="utf-8") as f:
        f.write(html_content)
        
    p_info = page.evaluate("""() => {
        const ps = Array.from(document.querySelectorAll('#TextContent p'));
        return ps.map((p, idx) => ({
            idx,
            classes: Array.from(p.classList),
            text: p.innerText.substring(0, 30)
        }));
    }""")
    
    print(f"Total paragraphs: {len(p_info)}")
    for p in p_info[:30]:
        print(f"P {p['idx']:03d}: classes={p['classes']}, text={p['text']}")
        
    page.close()
    browser.close()
